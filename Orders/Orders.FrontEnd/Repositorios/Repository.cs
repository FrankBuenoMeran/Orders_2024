    ...
    public class Repository : IRepository
    {
        ...
        public async Task<HttpResponseWrapper<T>> GetAsync<T>(string url)
        {
          var responseHttp= await _httpClient.GetAsync(url);
            ...
            {   //Serialirar es cunado tenemos un objecto en memoria y lo queremos convertir a formato Json
                ...
                var response = await UnserializeAnswerasync<T>(responseHttp);//Control punto a qui UnserializeAnswer para que nos cree la firma del metodo
        ...
        public async Task<HttpResponseWrapper<TActionResponse>> PostAsync<T, TActionResponse>(string url, T model)
        {
            ...
            {   //Serialirar es cunado tenemos un objecto en memoria y lo queremos convertir a formato Json
                //Deserializar es por que la respuesta viene en strig
                //Deserializamos la respueta
                var response = await UnserializeAnswerasync<TActionResponse>(responseHttp);//Control punto a qui UnserializeAnswer para que nos cree la firma del metodo
